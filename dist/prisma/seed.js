...
exports.createSeatsForShowtime = createSeatsForShowtime;
// prisma/seed.ts
...
async function main() {
    ...
    const halls = await Promise.all(hallsData.map(async (h) => {
    ...
    for (let dayOffset = 0; dayOffset < 3; dayOffset++) {
        for (const movie of movies) {
            for (const hour of timeSlots.slice(0, 2)) {
                ...
                const endsAt = new Date(startsAt.getTime() + movie.durationMin * 60 * 1000);
...
async function createSeatsForShowtime(showtimeId, rows, columns) {
    const seats = [];
